            ...
            for i, solution in enumerate(unique_solutions):
                ...
                if STN_lower_fit_limit is not None:
                    if current_fitness < STN_lower_fit_limit:
                        ...
                        continue
                start_node = (i == 0)
                end_node   = (i == len(unique_solutions) - 1)
                solution_tuple = tuple(solution)
                node_label = f"STN_run{run_idx}_step{i}"
                node_labels.append(node_label)
                # Save the mapping if you still need it later
                stn_node_mapping[(run_idx, i)] = node_label
                G.add_node(
                    node_label,
                ...
                # Add the noisy node for STN data
                noisy_node_label = f"Noisy {node_label}"
                G.add_node(
                    noisy_node_label,
                    solution=solution_tuple,
                    fitness=noisy_fitnesses[i],
                    type="STN_noisy"
                )
                G.add_edge(node_label, noisy_node_label, weight=STN_edge_size_slider,
                        color='gray', edge_type='Noise')

            # Connect consecutive nodes using zip
            for src_label, tgt_label in zip(node_labels, node_labels[1:]):
                G.add_edge(src_label, tgt_label, weight=STN_edge_size_slider,
                        color=edge_color, edge_type='STN')
