...
# ---------- PLOTTING -----------
    ...
    if plot_type == 'RegLon' or plot_type == 'NLon_box':
        ...
        for u, v, key, data in G.edges(data=True, keys=True):
            ...
            # # Compute values for arrows
            # source_point = np.array([pos[u][0], pos[u][1], G.nodes[u]['fitness']])
            # target_point = np.array([pos[v][0], pos[v][1], G.nodes[v]['fitness']])
            # vec = target_point - source_point
            # edge_length = np.linalg.norm(vec)
            
            # arrow_factor = 0.1  # Arrow length is 10% of the edge length.
            # arrow_length = arrow_factor * edge_length
            # norm_vec = vec / edge_length
            # arrow_vec = norm_vec * arrow_length
            # # Place the cone so that its tip aligns with the target node.
            # arrow_base = target_point - arrow_vec
            
            # arrow_x.append(arrow_base[0])
            # arrow_y.append(arrow_base[1])
            # arrow_z.append(arrow_base[2])
            ...
            plot_arrows = True
            arrow_factor = 0.1
            static_arrow_length = 0.2
            if plot_arrows == True and "Noisy" not in v:
                if option_curve_edges == True and "STN" in data.get("edge_type", ""):
                    # Using the curved path
                    # 'curve' is computed from quadratic_bezier and 'z_values' is computed via np.linspace
                    target_point = np.array([curve[-1, 0], curve[-1, 1], z_values[-1]])
                    # Use the difference between the last two points on the curve as the local direction:
                    if len(curve) >= 2:
                        second_last_point = np.array([curve[-2, 0], curve[-2, 1], z_values[-2]])
                    else:
                        second_last_point = np.array([curve[0, 0], curve[0, 1], z_values[0]])
                    vec = target_point - second_last_point
                    seg_length = np.linalg.norm(vec)
                    if seg_length == 0:
                        continue  # Skip zero-length segment
                    norm_vec = vec / seg_length
                    # arrow_length = arrow_factor * seg_length
                    arrow_vec = norm_vec * static_arrow_length
                    print("Arrow vector length:", np.linalg.norm(arrow_vec))
                    arrow_base = target_point - arrow_vec
                else:
                    # For straight edges (or non-curved)
                    source_point = np.array([pos[u][0], pos[u][1], G.nodes[u]['fitness']])
                    target_point = np.array([pos[v][0], pos[v][1], G.nodes[v]['fitness']])
                    vec = target_point - source_point
                    seg_length = np.linalg.norm(vec)
                ...
                # Accumulate arrow data for the cone trace
                arrow_x.append(arrow_base[0])
                arrow_y.append(arrow_base[1])
                arrow_z.append(arrow_base[2])
                arrow_u.append(arrow_vec[0])
        ...
        if arrow_x:
            cone_trace = go.Cone(
                        ...
                        v=arrow_v,
                        w=arrow_w,
                        anchor="tail",         # The cone starts at the base.
                        colorscale=[[0, data.get('color', 'red')], [1, data.get('color', 'red')]],     # Adjust or use a single color if desired.
                        showscale=False,
                        sizemode="absolute",
                        sizeref=1              # Adjust sizeref as needed.
                    )
